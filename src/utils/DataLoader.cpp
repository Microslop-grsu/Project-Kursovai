...
bool DataLoader::loadFromFile(const std::string& path,
    ...
    for (const auto& petJson: data["pets"] ) {
        if (!petJson.contains("type") || !petJson.contains("name")
            || !petJson.contains("age") || !petJson.contains("weight")) {
            continue;
        }
        ...
        if (type == "Exotic") {
            if (petJson.contains("temperature")) {
                double temp = petJson["temperature"];
