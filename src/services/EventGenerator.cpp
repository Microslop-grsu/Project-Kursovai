...
namespace
{
    ...
    std::vector<std::string> generateRandomHealthIssues(std::mt19937 &rng)
    {
        const std::vector<std::string> allIssues = {"obesity", "diabetes", "kidney_disease", "underweight"};
        ...
        { // 10% – две случайные
            std::vector<std::string> temp = allIssues;
            std::shuffle(temp.begin(), temp.end(), rng);
            result.assign(temp.begin(), temp.begin() + 2);
        }
        else if (p < 0.45)
        { // 30% – одну случайную
            std::uniform_int_distribution<size_t> idx(0, allIssues.size() - 1);
            result.push_back(allIssues[idx(rng)]);
        }
        // иначе 55% – без болезней
        return result;
    }
}
...
std::shared_ptr<SimulationEvent> EventGenerator::createAnimalArrival()
{
    ...
    auto healthIssues = generateRandomHealthIssues(randomEngine);
    return std::make_shared<AnimalArrivedEvent>(petData, healthIssues, "New intake");
}
